...
メモリレイアウトも、mul の演算順序と同様、クライアントプログラムからメモリを書き込んでもらうことになるので、ここも簡単には変えるわけにはいかない。
...
値が COLUMN_MAJOR とか書いてあるのでとてもわかり辛いが、slang が HLSL を源流としているためかもしれない。
...
### エントリポイントをグルーピングする ShaderPass

ShaderPass は「頂点シェーダー、フラグメントシェーダー、コンピュートシェーダーのエントリポイントの名前付き組み合わせ」です。

- パス名の **意味（Forward、ShadowCaster 等）は lumino_shader の関知するところではありません**。上位エンジンがパス名で問い合わせ、適切なパスを選択またはフォールバックします。
...
> **Slang 属性による代替案**: Slang のユーザー定義属性（`[LuminoPass("Forward", "vertex")]` のような形式）で同等の仕組みを構築することも技術的には可能ですが、現行の JSON 方式で十分に機能しており、シンプルさを優先して現在の設計を維持しています。
...
| ファイル | 役割 |
|---|---|
| `include/lumino_shader/Common.hpp` | 列挙型と共通構造体の定義 |
