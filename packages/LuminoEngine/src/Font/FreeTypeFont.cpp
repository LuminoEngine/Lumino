...
#define RESOLUTION_Y 96
    ...
    FT_Size ft_size;
    err = FTC_Manager_LookupSize(
        ftc_manager,
        &scaler,
        &ft_size); // FTC 使う場合は FT_Set_Char_Size の代わりにこれを使う。内部で FT_Set_Char_Size が呼ばれる。
    ...
    FT_UInt glyphIndex = FTC_CMapCache_Lookup(this->manager()->ftCacheMapCache(), m_ftFaceId, m_ftCacheMapIndex, 'a');
    if (LN_ENSURE(glyphIndex != 0, "failed FTC_CMapCache_Lookup")) return;

    FT_Glyph glyph;
    err = FTC_ImageCache_Lookup(this->manager()->ftCImageCache(), &m_ftImageType, glyphIndex, &glyph, NULL);
...
void FreeTypeFontCached::lookupGlyphBitmap(UTF32 utf32code, BitmapGlyphInfo* outInfo) {
    ...
    FT_Glyph glyph;
    FT_Error err = FTC_ImageCache_Lookup(manager()->ftCImageCache(), &m_ftImageType, glyphIndex, &glyph, NULL);
    if (LN_ENSURE(err == 0, "failed FTC_ImageCache_Lookup : %d\n", err)) return;
...
bool FreeTypeFontCached::getOutlineTextMetrix() {
    ...
    int tmInternalLeading =
        (FT_MulFix(os2->usWinAscent + os2->usWinDescent - m_ftFace->units_per_EM, y_scale) + 32) >> 6;
    int tmExternalLeading = std::max(
        0L,
        (FT_MulFix(
             hori->Line_Gap - ((os2->usWinAscent + os2->usWinDescent) - (hori->Ascender - hori->Descender)), y_scale) +
         32) >>
            6);
...
void FreeTypeFontCached::FT_BitmapToBitmap2D(FT_Bitmap* ftBitmap, Bitmap2D* bitmap) const {
    ...
    if (LN_REQUIRE(bitmap->width() >= width && bitmap->height() >= height && bitmap->format() == PixelFormat::A8))
        return;
...
Result<> FreeTypeFont::init(FontManager* manager, const FontDesc& desc) {
    ...
    FT_Error errCode = FT_New_Memory_Face(
        manager->ftLibrary(),
        (const FT_Byte*)faceSource->buffer->data(),
        faceSource->buffer->size(),
    ...
    errCode = FT_Stroker_New(manager->ftLibrary(), &m_ftStroker);
...
void FreeTypeFont::lookupGlyphBitmap(UTF32 utf32code, BitmapGlyphInfo* outInfo) {
...

    {
        ...
        // stroker を使いたいときは FT_LOAD_NO_BITMAP を使って、FT_GLYPH_FORMAT_OUTLINE を優先してロードしなければならない。
        FT_Int32 loadFlags = FT_LOAD_NO_BITMAP;
            //0 /*FT_LOAD_NO_BITMAP | FT_LOAD_NO_HINTING | FT_LOAD_NO_AUTOHINT | FT_LOAD_FORCE_AUTOHINT*/;
        if (outInfo->loadColor) {
            loadFlags |= FT_LOAD_COLOR;
        }

        // load glyph info to GlyphSlot(m_face->glyph), for access to metrix.
        result = FT_Load_Glyph(m_face, glyphIndex, loadFlags);
    ...
    if (glyphSlot->format == FT_GLYPH_FORMAT_BITMAP) {
        ...
        // glyphSlot の format が FT_GLYPH_FORMAT_BITMAP に変わる。
        //result = ::FT_Render_Glyph(glyphSlot, FT_RENDER_MODE_NORMAL);
        //if (LN_ENSURE(result == 0)) return;
    ...
    if (outlineWidth > 0.0f) {
        ...
        if (LN_ENSURE(result == FT_Err_Ok, "failed FT_Glyph_Copy : %d\n", result)) return;
        ...
        // glyph をコピーした新しい FT_Glyph を作成し、
        // stroker の設定を適用してそのポインタを glyph に格納する。
        result = FT_Glyph_StrokeBorder(
            &glyph,
            m_ftStroker,
            0,
            1   // コピー元の glyph を、FT_Glyph_StrokeBorder の中で FT_Done_Glyph　を使って削除するかどうか
        );
        if (LN_ENSURE(result == FT_Err_Ok, "failed FT_Glyph_StrokeBorder : %d\n", result)) return;

        result = FT_Glyph_To_Bitmap(&glyph, renderMode, NULL, 1);
        if (LN_ENSURE(result == FT_Err_Ok, "failed FT_Glyph_To_Bitmap : %d\n", result)) return;

        FT_BitmapGlyph glyph_bitmap = (FT_BitmapGlyph)glyph;
        FT_Bitmap* ft_bitmap = &glyph_bitmap->bitmap;
        outInfo->glyphBitmap = FTBitmapToInternalCacheBitmap(ft_bitmap);
        outInfo->size.width = glyphSlot->bitmap.width;
        outInfo->size.height = glyphSlot->bitmap.rows;
...
*/
    ...
    outMetrics->lineSpace = FLValueToFloatPx(
        m_face->size->metrics
            .height); // ascender - descender ではなく height を使う。FreeType 内部で端数が切り捨てられているので、1px足りないとかになる。
...
Bitmap2D* FreeTypeFont::FTBitmapToInternalCacheBitmap(FT_Bitmap* ftBitmap) {
    ...
    if (ftBitmap->pixel_mode == FT_PIXEL_MODE_MONO || ftBitmap->pixel_mode == FT_PIXEL_MODE_GRAY) {

        if (!m_internalCacheBitmapGray) {
            m_internalCacheBitmapGray =
                makeObject_deprecated<Bitmap2D>(ftBitmap->width, ftBitmap->rows, PixelFormat::A8);
        }
        else if (
            m_internalCacheBitmapGray->width() < ftBitmap->width ||
            m_internalCacheBitmapGray->height() < ftBitmap->rows) {
        ...
        if (!m_internalCacheBitmapRGBA) {
            m_internalCacheBitmapRGBA =
                makeObject_deprecated<Bitmap2D>(ftBitmap->width, ftBitmap->rows, PixelFormat::RGBA8);
        }
        else if (
            m_internalCacheBitmapRGBA->width() < ftBitmap->width ||
            m_internalCacheBitmapRGBA->height() < ftBitmap->rows) {
