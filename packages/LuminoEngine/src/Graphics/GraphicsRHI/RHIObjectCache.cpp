...
#include <LuminoEngine/Graphics/GraphicsRHI/RHIObjectCache.hpp>
...
void NativeRenderPassCache::invalidateRenderTarget(RHIResource* renderTarget) {
...
void NativeRenderPassCache::invalidateDepthBuffer(RHIResource* depthBuffer) {
    for (auto itr = m_hashMap.begin(); itr != m_hashMap.end();) {
