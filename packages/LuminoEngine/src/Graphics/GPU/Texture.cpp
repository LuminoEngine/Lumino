...
Result_deprecated<Ref<Texture2D>> Texture2D::createFromImageFileData(const uint8_t* data, int32_t length) {
...
// }
//
MaybeResult_deprecated Texture2D::init(Stream* stream, TextureFormat format) {
    ...
    if (m_modified) {
        if (m_rhiLockedBuffer) {
            ...
            detail::BitmapHelper::blitRawSimple(
                ...
                m_bitmap->width(),
                m_bitmap->height(),
                detail::BlitHelper::getPixelSize(m_bitmap->format()),
                device->caps().imageLayoytVFlip);

            if (!rhiObject || m_usage != rhiObject->usage() || mipmap() != rhiObject->mipmap()) {
                Ref<detail::RHIResource> ref = device->createTexture2D(m_usage, width(), height(), format(), mipmap(), bmpRawData);
...
void RenderTargetTexture::resetRHIObject(GraphicsContext* context, detail::RHIResource* rhiObject) {
    LN_CHECK(rhiObject);
    context->rhiResourceRegistry()->registerObject(this, rhiObject);
    auto size = rhiObject->extentSize();
    detail::TextureInternal::setDesc(this, size.width, size.height, rhiObject->textureFormat());
    m_modified = false;
}
...
Ref<Bitmap2D> RenderTargetTexture::readData(GraphicsContext* context) {
    bool modified = false;
    detail::RHIResource* rhiObject = resolveRHIObject2(context, &modified);
...
detail::RHIResource* RenderTargetTexture::resolveRHIObject(GraphicsCommandList* context, bool* outModified) {
    GraphicsContext* graphicsContext = context->graphicsContext();
    return resolveRHIObject2(graphicsContext, outModified);
}

detail::RHIResource* RenderTargetTexture::resolveRHIObject2(GraphicsContext* context, bool* outModified) {
    detail::RHIResource* rhiObject = static_cast<detail::RHIResource*>(context->rhiResourceRegistry()->get(this));
    *outModified = m_modified;

    if (m_modified) {
        detail::IGraphicsDevice* device = context->rhiDevice();
        Ref<detail::RHIResource> ref;
        if (m_hasNativeObject) {
            ref = device->createWrappedRenderTarget(m_nativeObject, width(), height());
        }
        else {
            ref = device->createRenderTarget(width(), height(), format(), mipmap(), m_msaa);
        }
        context->rhiResourceRegistry()->registerObject(this, ref);
        rhiObject = ref;
    }
    ...
    *outModified = m_modified;
    ...
    if (m_modified) {
        if (m_rhiLockedBuffer) {
            ...
            if (!rhiObject || m_usage != rhiObject->usage()) {
                Ref<detail::RHIResource> ref = device->createTexture3D(m_usage, width(), height(), depth(), format(), mipmap(), m_bitmap->data());
                graphicsContext->rhiResourceRegistry()->registerObject(this, ref);
                rhiObject = ref;
            }
            else {
