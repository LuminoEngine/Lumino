...
void UILayoutElement::arrangeLayout(UILayoutContext* layoutContext, const Rect& localSlotRect) {
    ...
    const Vector2& marginOffset = m_finalStyle->actualOuterOffset();
    m_localPosition.x = localSlotRect.x + /*finalLocalRect.x + */ marginOffset.x + arrangeRect.x; // +m_finalStyle->borderThickness.left;
    m_localPosition.y = localSlotRect.y + /*finalLocalRect.y + */ marginOffset.y + arrangeRect.y;  // +m_finalStyle->borderThickness.top;
...
UIHAlignment UILayoutElement::getLayoutHAlignment() const {
    ...
    const bool nl = Math::isNaN(margin.left);
    const bool nr = Math::isNaN(margin.right);
    if (nl && nr)
        return UIHAlignment::Center;
    else if (nr)
        return UIHAlignment::Left;
    else if (nl)
        return UIHAlignment::Right;
    else
        return UIHAlignment::Stretch;
}

UIVAlignment UILayoutElement::getLayoutVAlignment() const {
    const Thickness& margin = getLayoutMargin();
    const bool nt = Math::isNaN(margin.top);
    const bool nb = Math::isNaN(margin.bottom);
    if (nt && nb)
        return UIVAlignment::Center;
    else if (nb)
        return UIVAlignment::Top;
    else if (nt)
        return UIVAlignment::Bottom;
    else
        return UIVAlignment::Stretch;
}
