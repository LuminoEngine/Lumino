...
Result<void> Material::updateBindGroup(rhi::Device* device) {
    ...
    if (materialBindGroupLayout_ && baseTextureView_ && sampler_) {
        u64 paramSize = (type_ == MaterialType::Unlit)
            ? sizeof(UnlitMaterialParamsUBO)
            : sizeof(BasicLitMaterialParamsUBO);

        rhi::BindGroupDesc bgDesc;
        bgDesc.layout = materialBindGroupLayout_.get();
        bgDesc.entries = {
            {0, paramBuffer_.get(), 0, paramSize, nullptr, nullptr},  // uniform buffer
...
Result<Ref<Material>> MaterialFactory::createMaterialFromShaderData(
    ...
    auto loadResult = shader::UnifiedShaderSerializer::loadFromData(shaderData, shaderDataSize);
    ...
    // Create material bind group layout (Set 1)
    ...
    u64 paramSize = (type == MaterialType::Unlit)
        ? sizeof(UnlitMaterialParamsUBO)
        : sizeof(BasicLitMaterialParamsUBO);
