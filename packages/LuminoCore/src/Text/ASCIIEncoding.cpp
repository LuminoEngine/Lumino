...
#if LN_USTRING32
bool ASCIIEncoding::ASCIIDecoder::convertToUTF32(const byte_t* input, size_t inBufferByteCount, UTF32* output, size_t outBufferCharCount, TextDecodeResult* outResult)
{
    LN_DCHECK(input);
    LN_DCHECK(output);
    LN_DCHECK(outResult);
    ...
    for (; i < inBufferByteCount; ++i) {
        ...
        // 普通の ASCII 文字
        if (ch <= 0x7F) {
            output[i] = static_cast<UTF32>(ch);
        }
        // 代替文字が指定されていればそれを使う
        else if (fallbackReplacementChar != 0x00) {
            output[i] = static_cast<UTF32>(fallbackReplacementChar);
        }
...
bool ASCIIEncoding::ASCIIEncoder::convertFromUTF32(const UTF32* input, size_t inputElementSize, byte_t* output, size_t outputByteSize, TextEncodeResult* outResult)
{
    LN_DCHECK(input);
    LN_DCHECK(output);
    LN_DCHECK(outResult);
