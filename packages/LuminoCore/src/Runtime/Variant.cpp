...
Array<Ref<Variant>>& Variant::list() {
    assert(m_type == VariantType::Array);
    return v_List;
}

const Array<Ref<Variant>>& Variant::list() const {
    assert(m_type == VariantType::Array);
    return v_List;
}
...
void Variant::assign(const Array<Ref<Variant>>& value) {
    if (!changeType(VariantType::Array)) {
        v_List.~Array();
    }

    new (&v_List) Array<Ref<Variant>>(value);
}
...
void Variant::serializeInternal3(Archive& ar, ArchiveNodeType loadType) {
