...
while (Engine::update())
{
    ...
    playerBody->setVelocity(velocity, 0);
}
```

今回は setVelocity を使い、剛体の速度を設定することでプレイヤーを移動しています。移動中は速度が 5.0 となっていますが、これは1秒間に 5.0 移動することを示します。


### setPosition による移動と setVelocity による移動
