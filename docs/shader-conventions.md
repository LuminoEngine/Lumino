...
## WebGL2 (GLSL ES 300) の制約

Web の既定バックエンドは WebGL2 (OpenGL ES 3.0) です
([graphics-conventions.md](graphics-conventions.md) の「バックエンドと機能の制約」)。
WebGL2 向けのコードは SPIR-V から GLSL ES 300 へ変換して生成します。

Y 軸の向きとクリップ空間 Z の範囲の違いは、この変換時に補正コードを注入して吸収します。
**そのためシェーダの書き方自体は他のバックエンドと同じ**で、NDC や UV の規約はそのまま通ります。
ただし次の点は WebGL2 で破綻するため、守ってください。

### テクスチャとサンプラーは 1 対 1 で宣言する

GLSL ES 300 には `Texture2D` と `SamplerState` を分離した形が無く、両者が結合した
`sampler2D` (combined sampler) しかありません。変換時に (テクスチャ, サンプラー) の組ごとに
`sampler2D` が 1 つ生成されるため、1 つのサンプラーを複数のテクスチャで使い回すと
組み合わせの数だけ生成され、テクスチャユニットを余計に消費します。

既存の命名規約どおり、テクスチャ 1 つに対してサンプラー 1 つを
`<テクスチャ名>Sampler` のペアで宣言してください
(「[名前の指定は「テクスチャ側」の名前](#名前の指定はテクスチャ側の名前)」参照)。

### ユニフォームブロック名は生成時に書き換わる

GLSL ES 300 には `layout(binding = N)` が無く、実行時は**ブロック名**でユニフォームブロックを
特定します。SPIRV-Cross が既定で付けるブロック名は Slang の型名由来で `(set, binding)` から
導けないため、変換時に `_ln_ub_<set>_<binding>` という規則的な名前へ書き換えています。
シェーダを書く側から見た影響はありません (ブロック名はシェーダのコードからは参照しないため)
が、生成された GLSL を読むときは、この名前が Lumino が付けたものであることに注意してください。

### フラグメントシェーダで `SV_POSITION` を読まない

GLSL ES 300 には `gl_FragCoord` の原点を指定する `origin_upper_left` がありません。
画面座標が必要な場合は、頂点シェーダから UV として渡してください。

### コンピュートシェーダとストレージバッファは使えない

| 使えないもの | 代替 |
|---|---|
| コンピュートシェーダ | フラグメントシェーダで書く |
| ストレージバッファ (SSBO) | 定数バッファ (UBO) に置く。ES 3.0 の `MAX_UNIFORM_BLOCK_SIZE` の保証下限は 16KB で、`float4x4` なら 256 本に相当する |
| 5 本以上のカラー出力 | 4 本まで (`LN_MAX_COLOR_ATTACHMENTS`) |
...
- [ ] テクスチャとサンプラーを 1 対 1 のペアで宣言したか（サンプラーの使い回しをしていないか）
- [ ] フラグメントシェーダで `SV_POSITION` を読んでいないか（必要なら UV を渡す）
- [ ] コンピュートシェーダやストレージバッファを使っていないか
      （上の 3 点は [WebGL2 (GLSL ES 300) の制約](#webgl2-glsl-es-300-の制約)）
