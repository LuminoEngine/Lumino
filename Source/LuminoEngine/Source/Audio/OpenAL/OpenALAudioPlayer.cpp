...
//==============================================================================
// OpenALAudioPlayerBase 
//==============================================================================

//------------------------------------------------------------------------------
OpenALAudioPlayerBase::OpenALAudioPlayerBase(OpenALAudioDevice* device)
	: AudioPlayer(device)
	, m_OpenALAudioDevice(device)
	, mEmitterState(NULL)
{
}

//------------------------------------------------------------------------------
OpenALAudioPlayerBase::~OpenALAudioPlayerBase()
{
    LN_SAFE_DELETE( mEmitterState );
}

//------------------------------------------------------------------------------
void OpenALAudioPlayerBase::Initialize(AudioStream* audioStream, bool enable3d)
{
...
//------------------------------------------------------------------------------
void OpenALAudioPlayerBase::setPosition(const Vector3& pos)
{
    if ( mEmitterState )
    {
...
//------------------------------------------------------------------------------
void OpenALAudioPlayerBase::setVelocity(const Vector3& v)
{
    if ( mEmitterState )
    {
...
//==============================================================================
// OpenALOnMemoryAudioPlayer
//==============================================================================

//------------------------------------------------------------------------------
OpenALOnMemoryAudioPlayer::OpenALOnMemoryAudioPlayer(OpenALAudioDevice* device)
	: OpenALAudioPlayerBase(device)
	, mSourceVoice(NULL)
{
}

//------------------------------------------------------------------------------
OpenALOnMemoryAudioPlayer::~OpenALOnMemoryAudioPlayer()
{
	if ( mSourceVoice )
	{
		mSourceVoice->Stop();
...
//------------------------------------------------------------------------------
void OpenALOnMemoryAudioPlayer::Initialize(AudioStream* audioStream, bool enable3d)
{
	OpenALAudioPlayerBase::Initialize(audioStream, enable3d);
...
//------------------------------------------------------------------------------
void OpenALOnMemoryAudioPlayer::SetVolume(float volume)
{
    OpenALAudioPlayerBase::SetVolume( volume );
    if ( mSourceVoice )
	{
...
//------------------------------------------------------------------------------
void OpenALOnMemoryAudioPlayer::SetPitch(float pitch)
{
    OpenALAudioPlayerBase::SetPitch( pitch );
    if ( mSourceVoice )
	{
...
//------------------------------------------------------------------------------
uint64_t OpenALOnMemoryAudioPlayer::GetPlayedSamples() const
{
    if ( mSourceVoice )
    {
        OpenAL_VOICE_STATE voice_state;
...
//------------------------------------------------------------------------------
void OpenALOnMemoryAudioPlayer::Play()
{
	Stop();
	...
	LN_COMCALL(m_OpenALAudioDevice->GetOpenAL()->CreateSourceVoice(&mSourceVoice, &format));

	LN_COMCALL(mSourceVoice->FlushSourceBuffers());
        
    // SourceVoiceに送信するデータ
    OpenAL_BUFFER submit = { 0 };
	...
	submit.Flags = OpenAL_END_OF_STREAM;

	// ループ再生する場合
	if ( mIsLoop )
	{
		submit.LoopCount	= OpenAL_LOOP_INFINITE;	// 無限ループ
	...
	{
		submit.LoopBegin = OpenAL_NO_LOOP_REGION;
	}
...
//------------------------------------------------------------------------------
void OpenALOnMemoryAudioPlayer::Stop()
{
    if ( mSourceVoice )
	{
...
//------------------------------------------------------------------------------
void OpenALOnMemoryAudioPlayer::Pause( bool isPause )
{
	// 再生中の場合
...
//------------------------------------------------------------------------------
bool OpenALOnMemoryAudioPlayer::Polling()
{
	// 再生中ではない場合は中断
	if (!mSourceVoice || !mIsPlaying) {
		return false;
    ...
    {
        m_OpenALAudioDevice->CalcEmitterState( mEmitterState );
		...
		LN_COMCALL(mSourceVoice->SetOutputMatrix(
			m_OpenALAudioDevice->GetMasteringVoice(),   // NULL でもいいみたいだけど一応
...
//==============================================================================
// OpenALStreamingAudioPlayer
//==============================================================================

//------------------------------------------------------------------------------
OpenALStreamingAudioPlayer::OpenALStreamingAudioPlayer(OpenALAudioDevice* device)
	: OpenALAudioPlayerBase(device)
	, mSourceVoice(NULL)
...
//------------------------------------------------------------------------------
OpenALStreamingAudioPlayer::~OpenALStreamingAudioPlayer()
{
    if ( mSourceVoice )
	{
		mSourceVoice->Stop();
...
//------------------------------------------------------------------------------
void OpenALStreamingAudioPlayer::Initialize(AudioStream* audioStream, bool enable3d)
{
	OpenALAudioPlayerBase::Initialize(audioStream, enable3d);
...
//------------------------------------------------------------------------------
void OpenALStreamingAudioPlayer::SetVolume(float volume)
{
    OpenALAudioPlayerBase::SetVolume( volume );
    if ( mSourceVoice )
	{
...
//------------------------------------------------------------------------------
void OpenALStreamingAudioPlayer::SetPitch(float pitch)
{
	OpenALAudioPlayerBase::SetPitch(pitch);
    if ( mSourceVoice )
	{
...
//------------------------------------------------------------------------------
uint64_t OpenALStreamingAudioPlayer::GetPlayedSamples() const
{
    if ( mSourceVoice )
    {
        OpenAL_VOICE_STATE voice_state;
...
//------------------------------------------------------------------------------
void OpenALStreamingAudioPlayer::Play()
{
	Stop();
...
//------------------------------------------------------------------------------
void OpenALStreamingAudioPlayer::Stop()
{
	if ( mSourceVoice )
	{
		LN_COMCALL(mSourceVoice->Stop());
...
//------------------------------------------------------------------------------
void OpenALStreamingAudioPlayer::Pause( bool isPause )
{
	// 再生中の場合
	if ( mSourceVoice && mIsPlaying )
	{
...
//------------------------------------------------------------------------------
bool OpenALStreamingAudioPlayer::Polling()
{
	// 再生中ではない場合は中断
	if ( !mSourceVoice || !mIsPlaying ) {
		return false;
...
//------------------------------------------------------------------------------
void OpenALStreamingAudioPlayer::_addNextBuffer()
{
	...
	if ( !mIsLoop && mReadCursor >= mCursorEndPos )//mAudioSource->getSourceDataSize() )
	{
		...
		buffer.Flags = OpenAL_END_OF_STREAM;
	}
