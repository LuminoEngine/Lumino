...
Ref<VmdData> VmdFile::load(Stream* stream)
{
	...
	for (uint32_t i = 0; i < allBoneKeyCount; ++i)
	{
		...
		if (itr == m_boneTrackMap.end())
		{
			...
			track.boneName = String::fromCString(vmdMotion.szBoneName, -1, Encoding::getEncoding(EncodingType::SJIS));
			track.frames.add(frame);
			
			m_boneTrackMap[vmdMotion.szBoneName] = vmdData->MotionData.getCount();
	...
	if (!reader.isEOF())
	{
		...
		for (uint32_t i = 0; i < allFaceKeyCount; ++i)
		{
			...
			if (itr == m_morphTrackMap.end())
			{
				...
				track.morphName = String::fromCString(vmdFace.szFaceName, -1, Encoding::getEncoding(EncodingType::SJIS));
				track.frames.add(frame);
				
				m_morphTrackMap[vmdFace.szFaceName] = vmdData->MorphData.getCount();
