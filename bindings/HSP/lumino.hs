...
%prm
outViewport
[out] outViewport : Viewport オブジェクトのハンドルを格納する変数
...
%prm
viewport, outList
[in]  viewport : Viewport オブジェクトのハンドル
[out] outList  : ViewportLayerList オブジェクトのハンドルを格納する変数
...
[in]  sceneNode  : ノードハンドル
[out] outVisible : 可視状態を格納する変数
...
sceneNode, outList
[in]  sceneNode : ノードハンドル
[out] outList   : SceneNodeList オブジェクトのハンドルを格納する変数
...
%prm
listObject, outCount
[in]  listObject : オブジェクトリストハンドル
[out] outCount   : 要素の数を格納する変数
...
[in] listObject : オブジェクトリストハンドル
[in] index      : インデックス(要素番号)
[in] itemPtr    : 設定するオブジェクト
...
listObject, index, itemPtr
[in] listObject : オブジェクトリストハンドル
[in] index      : item を挿入するインデックス
[in] itemPtr    : 挿入するオブジェクト
...
[in]  listObject : オブジェクトリストハンドル
[in]  itemPtr    : リストから削除するオブジェクト
[out] outRemoved : 要素を削除したかどうかを示す値 (削除できた場合は LN_TRUE) を格納する変数のアドレス。
