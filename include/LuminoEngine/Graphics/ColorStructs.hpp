...
public:
	...
	constexpr Color(float rgb, float a_ = 1.0f) noexcept : r(rgb), g(rgb), b(rgb), a(a_) { }
	...
	constexpr Color(const Color& color, float a_) noexcept : r(color.r), g(color.g), b(color.b), a(a_) { }
...
public:
	...
	/** 各要素を設定します。 */
	void set(float r_, float g_, float b_, float a_ = 1.0f) noexcept { r = r_; g = g_; b = b_; a = a_; }
