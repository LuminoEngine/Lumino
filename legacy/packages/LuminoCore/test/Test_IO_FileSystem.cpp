...
TEST_F(Test_IO_FileSystem, ErrorHandling) {
    auto r1 = FileSystem::readAllText(U"xxxx");
    auto message1 = r1.toString();
    ASSERT_TRUE(r1.isErr());

    // エラーを持っているときに unwrap したら、dump しつつクラッシュする。
    //r.unwrap();

    auto r2 = FileSystem::writeAllText(U"", U"text");
    auto message2 = r2.toString();
...
TEST_F(Test_IO_FileSystem, SimpleReadWrite) {
    //* [ ] can read/write all bytes.
    {
        ...
        auto data2 = FileSystem::readAllBytes(LN_TEMPFILE("file1")).unwrap();
    ...
    //* [ ] can read/write all text.
    {
        ...
        String str2 = FileSystem::readAllText(LN_TEMPFILE("file2")).unwrap();
