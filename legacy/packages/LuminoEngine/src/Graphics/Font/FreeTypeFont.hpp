...
class FreeTypeFontCached : public FontCore {
public:
    FreeTypeFontCached();
...
private:
    ...
    // NOTE: skia は FT_Stroker は使っていないようだった。
    // SkScalerContext_FreeType::generatePath() など、SkPath に変換して、
    // それを別途 Bitmap 化しているようだった。

    // NOTE: 用語注意: Outline という言葉は縁取り文字ではなく、
    // 頂点バッファのようなパス情報を表す。
};
