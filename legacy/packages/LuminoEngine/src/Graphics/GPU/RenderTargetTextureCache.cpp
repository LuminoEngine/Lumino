...
RenderTargetTexture* RenderTargetTextureCacheManager::requestObject(const SizeI& size, TextureFormat format, bool mipmap)
{
	...
	auto t = makeObject_deprecated<RenderTargetTexture>(size.width, size.height, format, mipmap, false);
	insertBase(key, t);
	return t;
}
...
DepthBuffer* DepthBufferCacheManager::requestObject(const SizeI& size)
{
	...
	auto t = makeObject_deprecated<DepthBuffer>(size.width, size.height);
	insertBase(key, t);
	return t;
}
