...
static void jo_gif_lzw_encode(ln::BinaryWriter* os, unsigned char *in, int len)
{
    jo_gif_lzw_t state;
    int maxcode = 511;
...
{
    ln::ByteBuffer palette;			// パレット (最大256色 * 3) RGB
	ln::ByteBuffer indexed_pixels;
...
void jo_gif_decode(void *o_buf, jo_gif_frame_t *fdata, jo_gif_frame_t *palette_frame)
{
    ...
    int num_pixels = (int)fdata->indexed_pixels.GetSize();
...
void jo_gif_write_frame(ln::BinaryWriter* os, jo_gif_t *gif, jo_gif_frame_t *fdata, jo_gif_frame_t *palette_optional, int frame, short delayCsec)
{
    short width = gif->width;
    short height = gif->height;
    // int size = width * height;
