...
#undef LN_WRITE_V3
    ...
    // テクスチャ
    {
        float l = sprite.srcRect.x;
        float t = sprite.srcRect.y;
        float r = (sprite.srcRect.x + sprite.srcRect.width);
        float b = (sprite.srcRect.y + sprite.srcRect.height);

        if (testFlag(sprite.flipFlags, detail::SpriteFlipFlags::FlipX)) {
            std::swap(l, r);
        }
        if (testFlag(sprite.flipFlags, detail::SpriteFlipFlags::FlipY)) {
            std::swap(t, b);
        }
