...
{
    AbstractWin32PlatformWindow* window = reinterpret_cast<AbstractWin32PlatformWindow*>(::GetPropW(hwnd, Win32PlatformWindowManager::PropWinProc));
...
Result Win32PlatformWindow::init(Win32PlatformWindowManager* windowManager, const WindowCreationSettings& settings)
{
    ...
    BOOL r = ::SetPropW(m_hWnd, Win32PlatformWindowManager::PropWinProc, this);
...
Result WrappedWin32PlatformWindow::init(Win32PlatformWindowManager* windowManager, intptr_t	windowHandle)
{
    ...
    m_originalWndProc = (WNDPROC)::GetWindowLongW(m_hWnd, GWLP_WNDPROC);
    ::SetWindowLongW(m_hWnd, GWLP_WNDPROC, (LONG)StaticWndProcHook);
...
void WrappedWin32PlatformWindow::dispose()
{
    if (m_originalWndProc) {
        ::SetWindowLongW(m_hWnd, GWLP_WNDPROC, (LONG)m_originalWndProc);
        m_originalWndProc = nullptr;
...
{
    WrappedWin32PlatformWindow* window = reinterpret_cast<WrappedWin32PlatformWindow*>(::GetPropW(hwnd, Win32PlatformWindowManager::PropWinProc));
...
{
    ::UnregisterClassW(WindowClassName, m_hInst);
}
