...
// transform
const Vector3 UIStyle::DefaultOrigin = Vector3(0.0f, 0.0f, 0.0f);
...
void UIStyle::mergeFrom(const UIStyle* other) {
    ...
    // layout transform
    if (other->origin.hasValue()) origin = other->origin.get();
        ...
        for (auto& name : *classList) {
            ...
            if (cs) {
                for (const auto& slot : cs->visualStateStyles()) {
                    auto r = m_groups.findIf([&slot](const auto& x) { return x.activeStateIndex >= 0 && String::compare(x.stateNames[x.activeStateIndex], slot.name, CaseSensitivity::CaseInsensitive) == 0; });
                    if (r) {
                        style->mergeFrom(slot.style);
                    }
                }
