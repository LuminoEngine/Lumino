...
bool ImGuiIntegration::init(UIFrameWindow* frameWindow) {
    ...
    m_dockManager = Ref<ImGuiDockManager>(LN_NEW ImGuiDockManager(), false);
    m_dockManager->m_imguiIntegration = this;
...
#if LN_USE_KANATA
    m_shader = detail::RenderingManager::instance()->builtinShader(BuiltinShader::ImGui);
#else
...
void ImGuiIntegration::renderContents() {

    ImGui::NewFrame();
    ImGuiIO& io = ImGui::GetIO();

    m_dockManager->updateDocks();

    m_frameWindow->onImGui();
...
void ImGuiIntegration::render(GraphicsCommandList* graphicsContext, RenderTargetTexture* target) {
    ...
    ShaderPass* shaderPass = m_shader->techniques()[0]->passes()[0];
    graphicsContext->setShaderPass(shaderPass);
...
#if LN_USE_KANATA
    GraphicsCommandList* descriptorAllocator = graphicsContext;
    ShaderDescriptor* descriptor = descriptorAllocator->allocateDescriptor(shaderPass, true);

    descriptor->setUniformBufferData(0, &mat_projection, sizeof(mat_projection));
...
bool ImGuiIntegration::handlePlatformEvent(const PlatformEventArgs& e) {
    ...
    switch (e.type) {
        case PlatformEventType::MouseDown: {
            ...
            if (e.mouse.button == MouseButtons::Middle) {
                button = 2;
            }
            if (e.mouse.button == MouseButtons::X1) {
                button = 3;
            }
            if (e.mouse.button == MouseButtons::X2) {
                button = 4;
            }
        ...
        case PlatformEventType::KeyChar:
            io.AddInputCharacter(e.key.keyChar);
            //return true;
            break;
        default:
            break;
...
bool ImGuiIntegration::handleUIEvent(UIEventArgs* e) {
    return m_dockManager->handleUIEvent(e);
}

//void ImGuiIntegration::setImGuiContext(UIImGuiContext* context) {
...
//// UIImGuiContext
//
//void UIImGuiContext::onGui() {
//}

//==============================================================================
// ImGuiDockManager

ImGuiDockManager::ImGuiDockManager() {
    m_dockSpaceWindowFlags =
...
void ImGuiDockManager::addMenuBar(ImGuiMenuBar* menuBar) {
    if (LN_REQUIRE(menuBar)) return;
    menuBar->m_key = "Menu:" + std::to_string(m_dockPanes.size());
    m_menuBars.push(menuBar);
    menuBar->m_dockManager = this;
}

void ImGuiDockManager::addToolBar(ImGuiToolBar* toolBar) {
    if (LN_REQUIRE(toolBar)) return;
    toolBar->m_key = "ToolBar:" + std::to_string(m_dockPanes.size());
    m_toolBars.push(toolBar);
    toolBar->m_dockManager = this;
}

void ImGuiDockManager::addDockPane(ImGuiDockPane* pane) {
    if (LN_REQUIRE(pane)) return;
    pane->m_key = "DockPane:" + std::to_string(m_dockPanes.size());
    m_dockPanes.push(pane);
    pane->m_dockManager = this;
}

bool ImGuiDockManager::handleUIEvent(UIEventArgs* e) {
...
void ImGuiDockManager::updateDocks() {

    // DockArea 用に、ウィンドウ全体に広がる背景用 Window をつくる
    ImGuiID imguiWindowID = ImGui::GetID("RootDockSpace");
    {

        // Full screen base pane style. (see demo: ShowExampleAppDockSpace())
        ...
        m_dockSpaceWindowFlags |= ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove;
    ...
    if (m_dockLayoutResetRequired) {
        ...
        // BeginDockLayout
        {
            ...
            ImGui::DockBuilderAddNode(imguiWindowID, m_dockSpaceWindowFlags);
            ImGui::DockBuilderSetNodeSize(imguiWindowID, ImGui::GetMainViewport()->Size);
        }

        ImGuiID nodes1[2];
        ImGui::DockBuilderSplitNode(imguiWindowID, ImGuiDir_Left, 0.1f, &nodes1[0], &nodes1[1]);
        ImGuiID nodes2[2];
...
UIFrameWindow* ImGuiDockPane::frameWindow() const {
    return m_dockManager->m_imguiIntegration->frameWindow();
}
