...
/** UIElement */
LN_CLASS()
class UIElement : public UILayoutElement {
    LN_OBJECT;
    LN_BUILDER;
...
public:
    ...
    template<typename T>
    T dataAs() const { return m_data->get<T>(); }
...
public: // TODO: internal protected
    ...
    virtual const String& elementName() const {
        static String name = _TT("UIElement");
        return name;
    }
    ...
    */
    ...
    template<class T>
    T* logicalParentAs() const { return static_cast<T*>(m_logicalParent); }
...
struct UIElement::BuilderCore : public AbstractBuilder<T, B, D> {
    ...
    /** height property */
    B& height(float value) {
        d()->height = value;
        return self();
    }

    /** height property */
    B& backgroundColor(const Color& value) {
