...
TEST_F(Test_Text_Win32CodePageEncoding, convertFromUTF32) {
    ...
    // Convert in bulk
    {
        ...
        ASSERT_EQ(0, strncmp(narrowStr, narrowBuf, 6));
...
TEST_F(Test_Text_Win32CodePageEncoding, convertToUTF32_Issue1) {
    ...
    const char narrowStr[] = { 0x45, 0x72, 0x72, 0x6f, 0x72, 0x3a, 0x20, 0x7d, 0x20, 0xe3, 0x81, 0x8c, 0xe7, 0x84, 0xa1, 0xe3, 0x81, 0x84, 0x0d, 0x0a };
    Char strBuf[1024] = {};
    ASSERT_EQ(true, decoder->convertToUTF32((const byte_t*)narrowStr, strlen(narrowStr), (UTF32*)strBuf, 1024, &result));
}
