...
#ifdef _WIN32
int StringHelper::vsprintf(char* out, int charCount, const char* format, va_list args) {
    return _vsnprintf_s(out, charCount, _TRUNCATE, format, args);
}
    ...
    TUnigned* outNumber)     // NULL 可 (文字数カウント用)
{
    ...
    if (str == NULL) {
        return NumberConversionResult::ArgsError;
    }
    ...
    for (;;) {
        ...
        TUnigned d;
        if (isdigit(*p)) {
            d = static_cast<TUnigned>(*p - '0');
        }
        else if ('A' <= *p && *p <= 'F') {
            d = static_cast<TUnigned>(*p - 'A' + 10);
        }
        else if ('a' <= *p && *p <= 'f') {
            d = static_cast<TUnigned>(*p - 'a' + 10);
        }
...
static double StrToD_L(const char* str, char** endptr, _locale_t locale) {
    return _strtod_l(str, endptr, locale);
}
static double StrToD_L(const wchar_t* str, wchar_t** endptr, _locale_t locale) {
    return _wcstod_l(str, endptr, locale);
}
...
static double StrToD_L(const char* str, char** endptr, locale_t locale) {
    return strtod_l(str, endptr, locale);
}
...
double StringHelper::toDouble(const TChar* str, int len, const TChar** outEndPtr, NumberConversionResult* outResult) {
    ...
    if (str == NULL) {
        if (outResult != NULL) {
            *outResult = NumberConversionResult::ArgsError;
        }
        return 0.0;
    }
