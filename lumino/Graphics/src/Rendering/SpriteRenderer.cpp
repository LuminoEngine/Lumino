...
void SpriteRenderer::onActiveRenderFeatureChanged(const detail::CameraInfo& mainCameraInfo)
{
...
void SpriteRenderer::submitBatch(GraphicsCommandList* context, detail::RenderFeatureBatchList* batchList)
{
#ifdef LN_RLI_BATCH
	LN_UNREACHABLE();
#else
...
void SpriteRenderer::renderBatch(GraphicsCommandList* context, RenderFeatureBatch* batch)
{
...
void SpriteRenderer::makeRenderSizeAndSourceRectHelper(Texture* texture, const Size& size, const Rect& sourceRect, Size* outSize, Rect* outSourceRect)
{
	LN_DCHECK(outSize);
...
void SpriteRenderer::prepareBuffers(GraphicsCommandList* context, int spriteCount)
{
	if (context) {
