...
RenderTargetTexture* FrameBufferCache::requestRenderTargetTexture(const SizeI& size, TextureFormat format, bool mipmap)
{
	RenderTargetTexture* obj = m_renderTargetCache->requestObject(size, format, mipmap);
	m_aliveRenderTargets.add(obj);
	return obj;
}

DepthBuffer* FrameBufferCache::requestDepthBuffer(const SizeI& size)
{
	DepthBuffer* obj = m_depthBufferCache->requestObject(size);
