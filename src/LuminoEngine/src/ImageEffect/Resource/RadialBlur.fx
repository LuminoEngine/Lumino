...
{
    //return tex2D(_LensflareOcclusionMap, input.UV);


    // FIXME: 本来は ln_MaterialTexture のサイズとするべきだが、現状 Posteffect でしか使わず、
    // in,out のサイズは一致するのでこのままとする。
	...
	for (int i = 0; i < samples; i++) 
	{
		...
		//color += tex2D(ln_MaterialTexture, UV * scale + radialOffset);
