...
void SSRImageEffectInstance::onRender(RenderingContext* context, RenderTargetTexture* source, RenderTargetTexture* destination)
{
    ...
    m_ssrMaterial_NormalAndDepthSampler->setTexture(g_viewNormalMap);
    m_ssrMaterial_ViewDepthSampler->setTexture(g_viewDepthMap);
    m_ssrMaterial_MetalRoughSampler->setTexture(g_viewMaterialMap);
