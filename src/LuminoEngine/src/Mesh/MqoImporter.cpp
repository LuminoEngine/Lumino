...
void MqoObject::Smoothing()
{
	...
	// 面法線を求める
	// 頂点に、それを参照している面をつめる
	for (int i = 0; i < m_mqoFaceList.size(); i++)
	{
...
{
	auto mesh = makeRef<MeshResource>();
	mesh->initialize(EngineDomain::getGraphicsManager(), MeshCreationFlags::None);
	mesh->setName(m_name);

	// 頂点バッファを作る
	// (頂点バッファにつめる頂点はグループが基点となる)
	mesh->resizeVertexBuffer(m_mqoFaceList.size() * 3);
	int iVertex = 0;
	for (int iFace = 0; iFace < m_mqoFaceList.size(); iFace++)
	{
		MqoFace* mqoFace = &m_mqoFaceList[iFace];
		LN_CHECK(mqoFace->vertexCount == 3);
		...
		for (int iPoint = 0; iPoint < mqoFace->vertexCount; iPoint++)
		{
			...
			if (group->outputVertexIndex < 0)	// 重複登録の対策。まだ1度も走査されていなければ頂点バッファに書き込む
			{
				...
				mesh->setVertex(iVertex, Vertex{
					v.position,
					normal,
					mqoFace->uv[iPoint],
					Color::White });	// TODO: 頂点色
	...
	// インデックスバッファを作る
	mesh->resizeIndexBuffer(m_mqoFaceList.size() * 3);
	int iIndex = 0;
	...
	//// インデックスバッファを作りつつ、MqoVertex::referenced を作る
	{
		...
		for (int iFace = 0; iFace < m_mqoFaceList.size(); iFace++)
		{
			...
			sec.primitiveCount += face.vertexCount - 2;
		}
		mesh->addMeshSection(sec);
	}
...
void MqoObject::makeMqoFaceRefsAndEdge()
{
	...
	m_importer->m_mqoFaceRefBuffer.resize(m_mqoFaceList.size() * 3);
	m_importer->m_mqoEdgeBuffer.resize(m_mqoFaceList.size() * 3);
	m_importer->m_mqoFacePointGroupBuffer.clearAndReserve(m_mqoFaceList.size() * 3);
	...
	// 隣接辺を検索する
	for (int iEdge = 0; iEdge < m_importer->m_mqoEdgeBuffer.size(); iEdge++)
	{
...
//------------------------------------------------------------------------------
Ref<MeshModel> MqoImporter::import(ModelManager* manager, const Path& parentDir, Stream* stream)
{
	...
	m_model = makeRef<MeshModel>();
	m_model->initialize(manager->getGraphicsManager());

	// Metasequoia4 で出力される .mqo ファイルの文字コードは Shift_JIS だった
	StreamReader reader(stream, TextEncoding::getEncoding(EncodingType::SJIS));
	...
	while (reader.readLine(&line))
	{
		...
		if (index > -1)
		{
			...
			loadObject(&reader, line.substr(nameBegin, nameEnd - nameBegin));
		}
	}
...
{
	auto mqoObject = makeRef<MqoObject>();
