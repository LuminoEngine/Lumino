...
void Model::Create(detail::ModelManager* manager, const PathName& filePath)
{
	...
	if (boneCount > 0)
	{
		...
		//RefPtr<Bitmap> bitmap(LN_NEW Bitmap(size, Utils::TranslatePixelFormat(TextureFormat::R32G32B32A32_Float)), false);
		m_skinningMatricesTexture->Initialize(m_manager->GetGraphicsManager(), size, TextureFormat::R32G32B32A32_Float, 1);
